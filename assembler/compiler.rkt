...
(define builtins
  '((+ ADD)
    ...
    (compiled-procedure-entry CLSR)
    ))

(define (compile-primitive-procedure operator target linkage)
  (end-with-linkage linkage
  ...
  (let ((proc-code (compile (operator exp) 'proc 'next))
        (operand-codes
         (map (lambda (operand) (compile operand 'val 'next))
              (operands exp))))
    (if (assoc (operator exp) builtins)
        (preserving '(env continue)
                    (construct-arglist operand-codes)
                    (compile-primitive-procedure (operator exp)
                                                 target
                                                 linkage))
        (preserving '(env continue)
                    proc-code
