...
void init(Vector *vector, int memorySize) {
    vector->array = (char *)malloc(memorySize * sizeof(char));
...
int insert(Vector *vector, int location, char c) {
    if (location < 0 || location > vector->size) {
        ...
        vector->array[location] = c;
        vector->size++;
        return 0;
    }
}

/* inserts a char to the end of the vector */
int append(Vector *vector, char c) {
    return insert(vector, vector->size, c);
}
...
int get(Vector *vector, int location, char *c) {
    if (location < 0 || location >= vector->size) {
        return 1;
    } else {
        *c = vector->array[location];
        return 0;
    }
}
